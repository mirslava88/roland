...
app.whenReady().then(async()=>{
 ...
 try{
  ...
  const rawStep=async n=>wait(()=>frame('d.querySelector(".interface-tour-card header span")?.textContent.includes("Шаг '+(n+1)+' из 32")'),'tour step '+(n+1))
  const step=rawStep
  ...
  const restoredStateMatches=async index=>frame(`(()=>{const s=w.trainingStore.getState(),a=s.channelIds[0],b=s.channelIds[1];switch(${index}){
   ...
   case 29:return !d.querySelector("[data-pdm-training-panel=settings]");
   case 30:return !d.querySelector("[data-pdm-training-panel=stream]");
   case 31:return !!s.activeFile&&s.programScene.enabled;
   default:return false;
  }})()`)
  const verifyEveryBackRestoration=async()=>{
   for(let current=1;current<=31;current++){
    ...
    const optionalStream=previous===30&&await frame('!d.querySelector("[data-toolbar-item=stream]")')
  ...
  const nextAfterDone=async nextIndex=>{
   ...
   assert.equal(await frame('w.getComputedStyle(d.querySelector(".interface-tour-next")).borderTopColor==="rgb(255, 213, 79)"'),true,'The required Next button is highlighted in yellow')
   await click('.interface-tour-next');await step(nextIndex)
  }
  const inspectPanel=async(index,button,panel)=>{
   await wait(()=>frame('!!d.querySelector("[data-pdm-training-action-button-outline]")'),'panel opener highlighted for '+panel)
   assert.equal(await frame('(()=>{const b=d.querySelector('+JSON.stringify(button)+')?.getBoundingClientRect(),o=d.querySelector("[data-pdm-training-action-button-outline]")?.getBoundingClientRect();return !!b&&!!o&&Math.abs(b.left-o.left)<=7&&Math.abs(b.top-o.top)<=7&&Math.abs(b.right-o.right)<=7&&Math.abs(b.bottom-o.bottom)<=7})()'),true,'Required panel button is highlighted in yellow')
   ...
   await wait(()=>frame('!!d.querySelector("[data-pdm-training-action-button-outline]")'),'panel close highlighted for '+panel)
   assert.equal(await frame('(()=>{const b=d.querySelector('+JSON.stringify(panel+' [data-pdm-training-close]')+')?.getBoundingClientRect(),o=d.querySelector("[data-pdm-training-action-button-outline]")?.getBoundingClientRect();return !!b&&!!o&&Math.abs(b.left-o.left)<=7&&Math.abs(b.top-o.top)<=7&&Math.abs(b.right-o.right)<=7&&Math.abs(b.bottom-o.bottom)<=7})()'),true,'Required close button is highlighted in yellow')
  ...
  await screen('actual-channels')
   ...
   await pause(1200)
   assert.ok(await frame('d.querySelector(".interface-tour-card header span")?.textContent.includes("Шаг 2 из 32")'),'Step 2 result must remain readable before step 3')
   await nextAfterDone(2)
   await wait(()=>frame('!!d.querySelector("[data-pdm-training-action-button-outline]")'),'first channel highlighted')
   assert.equal(await frame('(()=>{const b=d.querySelector(".pdm-channel-card:nth-child(1)")?.getBoundingClientRect(),o=d.querySelector("[data-pdm-training-action-button-outline]")?.getBoundingClientRect();return !!b&&!!o&&Math.abs(b.left-o.left)<=7&&Math.abs(b.top-o.top)<=7&&Math.abs(b.right-o.right)<=7&&Math.abs(b.bottom-o.bottom)<=7})()'),true,'Step 3 highlights the whole first channel in yellow')
   await click('.pdm-channel-card:nth-child(1)');await step(3)
   await click('[data-toolbar-item="output"] button');await step(4)
   await wait(()=>frame('!!d.querySelector("[data-pdm-training-action-button-outline]")'),'second channel highlighted')
   assert.equal(await frame('(()=>{const b=d.querySelector(".pdm-channel-card:nth-child(2)")?.getBoundingClientRect(),o=d.querySelector("[data-pdm-training-action-button-outline]")?.getBoundingClientRect();return !!b&&!!o&&Math.abs(b.left-o.left)<=7&&Math.abs(b.top-o.top)<=7&&Math.abs(b.right-o.right)<=7&&Math.abs(b.bottom-o.bottom)<=7})()'),true,'Step 5 highlights the whole second channel in yellow')
   ...
   await wait(()=>frame('!!d.querySelector("[data-pdm-training-action-button-outline]")'),'second channel Take button highlighted')
   assert.equal(await frame('(()=>{const b=d.querySelectorAll(".pdm-channel-card")[1]?.querySelector("[data-pdm-channel-take]")?.getBoundingClientRect(),o=d.querySelector("[data-pdm-training-action-button-outline]")?.getBoundingClientRect();return !!b&&!!o&&Math.abs(b.left-o.left)<=7&&Math.abs(b.top-o.top)<=7&&Math.abs(b.right-o.right)<=7&&Math.abs(b.bottom-o.bottom)<=7&&!d.querySelector(".interface-tour-outline")})()'),true,'Step 6 highlights the channel Take button only in yellow')
   ...
   await wait(()=>frame('!!d.querySelector("[data-pdm-training-action-button-outline]")'),'second slide highlighted')
   assert.equal(await frame('(()=>{const b=d.querySelectorAll(".pdm-slide-thumb")[1]?.getBoundingClientRect(),o=d.querySelector("[data-pdm-training-action-button-outline]")?.getBoundingClientRect();return !!b&&!!o&&Math.abs(b.left-o.left)<=7&&Math.abs(b.top-o.top)<=7&&Math.abs(b.right-o.right)<=7&&Math.abs(b.bottom-o.bottom)<=7&&!d.querySelector(".interface-tour-outline")})()'),true,'Step 7 highlights only the second slide in yellow')
   assert.equal(await frame('(()=>{const s=d.querySelectorAll(".pdm-slide-thumb")[0].style;return s.getPropertyValue("border-color")==="transparent"&&s.getPropertyPriority("border-color")==="important"})()'),true,'Step 7 removes the normal selection border from the first slide')
  ...
  assert.equal(await frame('!!d.querySelector("[data-pdm-display-modal]")'),false,'Screens overview closes before Scene')
   ...
   await wait(()=>frame('!!d.querySelector("[data-pdm-training-right-click-guide]")'),'yellow right-click mouse guide')
   assert.equal(await frame('(()=>{const p=d.querySelector("[data-program-scene-preview]").getBoundingClientRect(),g=d.querySelector("[data-pdm-training-right-click-guide]").getBoundingClientRect(),m=d.querySelector(".interface-tour-mouse-picture"),r=w.getComputedStyle(m);return g.left>=p.left&&g.right<=p.right&&g.top>=p.top&&g.bottom<=p.bottom&&r.borderTopColor==="rgb(255, 213, 79)"})()'),true,'Step 18 draws a yellow mouse and right-click instruction directly on the Scene preview')
   await rightClickFrame('[data-program-scene-preview]');await wait(()=>frame('!d.querySelector("[data-pdm-training-right-click-guide]")'),'right-click mouse guide hidden after menu opens');await step(17)
  assert.equal(await frame('(()=>{const b=d.querySelector("[data-scene-add=text]")?.getBoundingClientRect(),o=d.querySelector("[data-pdm-training-action-button-outline]")?.getBoundingClientRect();return !!b&&!!o&&Math.abs(b.left-o.left)<=7&&Math.abs(b.top-o.top)<=7&&Math.abs(b.width-o.width)<=12&&Math.abs(b.height-o.height)<=12})()'),true,'The required Text command is highlighted in yellow before it is selected')
  ...
  assert.equal(await frame('JSON.stringify(w.trainingStore.getState().programSnapshot.scene.textOverlays)'),published,'Draft must not leak to program')
   ...
   await wait(()=>frame('!!d.querySelector("[data-pdm-training-right-click-guide].is-compact")'),'compact chroma right-click mouse guide')
   assert.equal(await frame('(()=>{const p=d.querySelector("[data-program-scene-participant-preview]").getBoundingClientRect(),g=d.querySelector("[data-pdm-training-right-click-guide]").getBoundingClientRect(),m=d.querySelector(".interface-tour-mouse-picture").getBoundingClientRect();return g.left+g.width/2>=p.left&&g.left+g.width/2<=p.right&&g.top+g.height/2>=p.top&&g.top+g.height/2<=p.bottom&&m.width<50&&m.height<65})()'),true,'Step 23 draws a small yellow right-click mouse directly on the camera')
   ...
   await wait(()=>frame('!d.querySelector("[data-pdm-training-right-click-guide]")'),'chroma right-click mouse guide hidden after menu opens')
   await click('[data-scene-object-action="chroma"]')
   await wait(()=>frame('(()=>{const b=d.querySelector("[data-program-scene-chroma-toggle]")?.closest("label")?.getBoundingClientRect(),o=d.querySelector("[data-pdm-training-action-button-outline]")?.getBoundingClientRect();return !!b&&!!o&&Math.abs(b.left-o.left)<=7&&Math.abs(b.top-o.top)<=7&&Math.abs(b.right-o.right)<=7&&Math.abs(b.bottom-o.bottom)<=7})()'),'chroma enabled button highlighted')
   assert.equal(await frame('(()=>{const b=d.querySelector("[data-program-scene-chroma-toggle]")?.closest("label")?.getBoundingClientRect(),o=d.querySelector("[data-pdm-training-action-button-outline]")?.getBoundingClientRect();return !!b&&!!o&&Math.abs(b.left-o.left)<=7&&Math.abs(b.top-o.top)<=7&&Math.abs(b.right-o.right)<=7&&Math.abs(b.bottom-o.bottom)<=7&&!d.querySelector(".interface-tour-outline")})()'),true,'Step 23 highlights the chroma Enabled control only in yellow')
   await click('[data-program-scene-chroma-toggle]');await screen('actual-chroma-training')
   ...
   await wait(()=>frame('!!d.querySelector("[data-pdm-training-right-click-guide].is-compact")'),'compact QR right-click mouse guide')
   assert.equal(await frame('(()=>{const p=d.querySelector("[data-program-scene-preview]").getBoundingClientRect(),g=d.querySelector("[data-pdm-training-right-click-guide]").getBoundingClientRect(),m=d.querySelector(".interface-tour-mouse-picture").getBoundingClientRect();return g.left+g.width/2>=p.left&&g.left+g.width/2<=p.right&&g.top+g.height/2>=p.top&&g.top+g.height/2<=p.bottom&&m.width<50&&m.height<65})()'),true,'Step 24 draws a small yellow right-click mouse directly on the Scene preview')
   await rightClickFrame('[data-program-scene-preview]');await wait(()=>frame('!d.querySelector("[data-pdm-training-right-click-guide]")'),'QR right-click mouse guide hidden after menu opens');await wait(()=>frame('!!d.querySelector("[data-scene-add=qr]")'),'QR command in real context menu')
   await click('[data-scene-add="qr"]');await wait(()=>frame('!!d.querySelector("[data-pdm-training-qr-url]")'),'QR URL field')
   await wait(()=>frame('!!d.querySelector("[data-pdm-training-required-field-outline]")'),'QR URL field highlighted')
   assert.equal(await frame('(()=>{const b=d.querySelector("[data-pdm-training-qr-url]")?.getBoundingClientRect(),o=d.querySelector("[data-pdm-training-required-field-outline]")?.getBoundingClientRect();return !!b&&!!o&&Math.abs(b.left-o.left)<=7&&Math.abs(b.top-o.top)<=7&&Math.abs(b.right-o.right)<=7&&Math.abs(b.bottom-o.bottom)<=7&&!d.querySelector(".interface-tour-outline")})()'),true,'Step 24 highlights the URL field only in yellow')
   ...
   assert.ok(await frame('d.querySelector(".interface-tour-card header span")?.textContent.includes("Шаг 24 из 32")'),'QR lesson must not advance while the address field is still focused')
   ...
   assert.equal(await frame('(()=>{const q=d.querySelector("[data-scene-qr-object]").getBoundingClientRect(),o=d.querySelector("[data-pdm-training-action-button-outline]")?.getBoundingClientRect();return !!o&&Math.abs(q.left-o.left)<=7&&Math.abs(q.top-o.top)<=7&&Math.abs(q.width-o.width)<=12&&Math.abs(q.height-o.height)<=12&&!d.querySelector(".interface-tour-outline")})()'),true,'Ready QR becomes the yellow highlighted drag target')
   ...
   assert.ok(await frame('d.querySelector(".interface-tour-card header span")?.textContent.includes("Шаг 25 из 32")'),'Titles lesson must stay on the same step after the first name character')
   ...
   assert.ok(await frame('d.querySelector(".interface-tour-card header span")?.textContent.includes("Шаг 25 из 32")'),'Titles lesson must not advance while the event field is still focused')
   ...
   await wait(()=>frame('!!d.querySelector("[data-pdm-training-action-button-outline]")'),'title preview highlighted')
   assert.equal(await frame('(()=>{const p=d.querySelector("[data-broadcast-titles-preview]").getBoundingClientRect(),o=d.querySelector("[data-pdm-training-action-button-outline]").getBoundingClientRect();return Math.abs(p.left-o.left)<=7&&Math.abs(p.top-o.top)<=7&&Math.abs(p.width-o.width)<=12&&Math.abs(p.height-o.height)<=12&&!d.querySelector(".interface-tour-outline")})()'),true,'Completed title fields move the yellow frame to the required title preview')
   ...
   await wait(()=>frame('!!d.querySelector("[data-pdm-training-action-button-outline]")'),'Settings button highlighted')
   assert.equal(await frame('(()=>{const b=d.querySelector("[data-toolbar-item=settings] button")?.getBoundingClientRect(),o=d.querySelector("[data-pdm-training-action-button-outline]")?.getBoundingClientRect();return !!b&&!!o&&Math.abs(b.left-o.left)<=7&&Math.abs(b.top-o.top)<=7&&Math.abs(b.right-o.right)<=7&&Math.abs(b.bottom-o.bottom)<=7&&!d.querySelector(".interface-tour-outline")})()'),true,'Step 29 highlights Settings in yellow')
   ...
   await wait(()=>frame('!!d.querySelector("[data-pdm-training-required-field-outline]")'),'projector audio output outlined')
   assert.equal(await frame('(()=>{const t=d.querySelector("[data-pdm-audio-device=training-hdmi]")?.getBoundingClientRect(),o=d.querySelector("[data-pdm-training-required-field-outline]")?.getBoundingClientRect();return !!t&&!!o&&Math.abs(t.left-o.left)<=7&&Math.abs(t.top-o.top)<=7&&Math.abs(t.right-o.right)<=7&&Math.abs(t.bottom-o.bottom)<=7})()'),true,'Projector / HDMI audio output is highlighted with a yellow outline')
   ...
   await wait(()=>js('document.querySelector(".intro-interface-header span").textContent.includes("Виртуальная камера")'),'automatic transition to virtual camera lesson')
   assert.ok(await frame('!!d.querySelector("[data-pdm-training-panel=settings]")'),'Settings stay open between audio output and virtual camera lessons')
   await wait(()=>frame('d.querySelector(".interface-tour-card header span")?.textContent.includes("Шаг 30 из 32")'),'standard step 30 card visible')
   assert.ok(await frame('d.querySelector(".interface-tour-card p")?.textContent.includes("Нажмите вкладку")'),'Standard lesson card explains how to open the virtual camera tab')
   assert.ok(await frame('d.querySelector(".interface-tour-card p")?.textContent.includes("передаёт итоговое изображение PDM")'),'Step 30 briefly explains what the virtual camera does')
   await wait(()=>frame('!!d.querySelector("[data-pdm-training-required-field-outline]")'),'virtual camera tab outlined')
   const virtualCameraTabOutlineMatches = await frame(`(()=>{const t=d.querySelector('[data-pdm-settings-tab="virtual-camera"]')?.getBoundingClientRect();const o=d.querySelector('[data-pdm-training-required-field-outline]')?.getBoundingClientRect();return !!t&&!!o&&Math.abs(t.left-o.left)<=7&&Math.abs(t.top-o.top)<=7&&Math.abs(t.right-o.right)<=7&&Math.abs(t.bottom-o.bottom)<=7})()`)
   assert.ok(virtualCameraTabOutlineMatches,'Virtual camera tab is highlighted with a yellow outline')
   await click('[data-pdm-settings-tab="virtual-camera"]')
   await wait(()=>frame('!!d.querySelector("[data-pdm-virtual-camera-settings]")'),'virtual camera settings visible')
   await wait(()=>frame('d.querySelector(".interface-tour-card p")?.textContent.includes("Включить камеру")'),'standard step card explains how to enable the virtual camera')
   assert.equal(await frame('(()=>{const b=d.querySelector("[data-pdm-virtual-camera-start]")?.getBoundingClientRect(),c=d.querySelector(".interface-tour-card")?.getBoundingClientRect();return !!b&&!!c&&(c.right<=b.left||c.left>=b.right||c.bottom<=b.top||c.top>=b.bottom)})()'),true,'Step 30 card must not cover the Enable camera button')
   assert.equal(await frame('(()=>{const t=d.querySelector("[data-pdm-virtual-camera-start]")?.getBoundingClientRect(),o=d.querySelector("[data-pdm-training-required-field-outline]")?.getBoundingClientRect();return !!t&&!!o&&Math.abs(t.left-o.left)<=7&&Math.abs(t.top-o.top)<=7&&Math.abs(t.right-o.right)<=7&&Math.abs(t.bottom-o.bottom)<=7})()'),true,'Enable camera button is highlighted after opening the tab')
   assert.ok(await frame('d.querySelector("[data-pdm-virtual-camera-settings]")?.textContent.includes("не передаёт звук")'),'Training explains that virtual camera audio is selected separately')
   await click('[data-pdm-virtual-camera-start]')
   await wait(()=>frame('d.querySelector("[data-pdm-virtual-camera-settings]")?.textContent.includes("Виртуальная камера включена")'),'training virtual camera enabled')
   await wait(()=>frame('d.querySelector(".interface-tour-card p")?.textContent.includes("закройте настройки")'),'standard step card explains how to close settings')
   await screen('actual-virtual-camera-training')
   await click('[data-pdm-training-panel="settings"] [data-pdm-training-close]')
   await nextAfterDone(30)
   if(await frame('!!d.querySelector("[data-toolbar-item=stream] button")')) await inspectPanel(30,'[data-toolbar-item="stream"] button','[data-pdm-training-panel="stream"]')
   else await step(31)
  ...
  assert.ok(await js('document.querySelector(".intro-primary").textContent.includes("Продолжить")'))
   console.log('PASS: 32-step real PDM tour teaches multiple channels, seamless TAKE, media, Scene, external camera, chroma key, QR, titles, audio output, virtual camera and stream; draft isolation, resume and real session/IPC safety hold')
