...
assert.match(sceneModal, /draft\.duration <= 0[\s\S]*minutes <= 0[\s\S]*return draft[\s\S]*const duration = minutes \* 60/)
...
assert.match(previewPanel, /const \{ width, height \} = pdfZoomRenderSize\(cssWidth, cssHeight, pixelRatio, zoomRenderScale\)/)
assert.match(previewPanel, /canvas\.style\.width = `\$\{cssWidth\}px`/)
