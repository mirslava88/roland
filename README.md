...
- Живая копия внешнего источника теперь получает камеру, плату видеозахвата, окно или экран напрямую: изображение не чернеет, звук не дублируется, а титры и наложенный таймер сохраняются поверх копии.
- После временного переключения входа дополнительного монитора его роль и назначение сохраняются; при возвращении того же дисплея PDM автоматически открывает нужное окно и восстанавливает живую копию.
- Титры внешнего источника синхронизируются во всех каналах, основном эфире и информационных экранах, где выбран один и тот же источник. Показ титра из карточки канала больше не переключает канал и не запускает TAKE.
- На информационном экране титры можно выбрать прямо из его предварительного просмотра. Исправлены скрытые под окном «Экраны» диалоги выбора внешнего устройства, окна и экрана.
- Обнаружение мониторов защищено от кратковременных неполных снимков топологии Windows, а системная панель задач ноутбука больше не скрывается из-за устаревших координат отключённого дисплея.
- Живая копия удерживает последний готовый кадр до первого реально отрисованного кадра нового источника и автоматически повторяет подключение после временного отказа видеопотока.
