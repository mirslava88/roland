...
```text
Дисплей 0 — интерфейс оператора
Дисплей 1 — основной эфир для зрителей
Дисплей 2 — суфлёр: текущий и следующий слайды, заметки PPTX
Дисплей 3 — независимый PPTX/PDF/видеоролик/изображение или таймер
```
...
- **Изображения и подложка** — выбранный фон применяется ко всем включённым дополнительным дисплеям и не переносится в следующую сессию. На суфлёре фон служит чистой заглушкой, когда в эфире нет PPTX/PDF, и не просвечивает между блоками слайдов.
...
- **Суфлёр докладчика** — отдельный дисплей следует за активной PPTX/PDF-презентацией и показывает текущий слайд, следующий слайд, номер и заметки PowerPoint.
- **Информационный экран** — независимый показ PPTX, PDF, видео или изображения; его содержимое и навигация не меняются при переключении каналов основного эфира.
- **Выбор экрана для таймера** — таймер выводится либо поверх основного эфира на Дисплее 1, либо только на информационном Дисплее 3.
...
5. Во вкладке **«Дисплеи»** назначьте физические мониторы ролям PDM: Дисплей 1 — эфир, Дисплей 2 — суфлёр, Дисплей 3 — информационный экран.
...
Во вкладке **«Настройки → Дисплеи»** каждому физическому монитору назначается одна роль. Нумерация внутри PDM не зависит от номеров экранов Windows:

- **Дисплей 0** — основной экран ноутбука с интерфейсом оператора;
- **Дисплей 1** — основной эфир для зрителей;
- **Дисплей 2** — суфлёр докладчика для PPTX и PDF;
- **Дисплей 3** — независимый мультимедийный и информационный экран.

Кнопка **«🖥 Экраны»** на верхней панели включает и скрывает суфлёр и информационный экран. Суфлёр автоматически следует только за презентацией, которая сейчас находится в эфире. Для Дисплея 3 можно отдельно выбрать PPTX, PDF, видеоролик или изображение; презентации листаются собственными кнопками, а видео запускается вручную. В настройках таймера выбирается одно место вывода: Дисплей 1 или только Дисплей 3. Если назначенный монитор отключён, служебное окно не переносится на другой дисплей.

### Информационный Дисплей 3

1. Назначьте физический монитор роли **Дисплей 3** во вкладке **«Настройки → Дисплеи»**.
2. Нажмите **«🖥 Экраны» → «Выбрать файл»** и загрузите PPT/PPTX, PDF, видео или изображение.
...
4. Видеоролик не запускается автоматически — используйте кнопку **«Воспроизвести»** и **«Пауза»**.
5. Кнопка **«Убрать файл с информационного экрана»** возвращает общую подложку, а если она не выбрана — чёрный фон.

Выбранный файл действует только в текущей сессии PDM. Если таймер направлен на Дисплей 3 и установлен, он временно занимает весь информационный экран; после остановки таймера снова отображается выбранный мультимедийный файл.

## Таймер

Таймер может открываться поверх основного эфира либо занимать отдельный информационный дисплей и не мешает управлению презентацией. Доступны:

- выбор места вывода: поверх основного эфира на Дисплее 1 или только на информационном Дисплее 3;
