...
- **Автовосстановление рабочей сессии** — подготовленные каналы, подписи, источники, выбранные слайды, сетка и текущая страница автоматически сохраняются между запусками. После перезапуска или аварийного завершения подготовка возвращается без автоматического выхода в эфир; если отдельный файл удалён или переименован, очищается только связанный с ним канал.
...
- **Живая копия эфира** — второй монитор с режимом «Основной эфир» зеркалирует главный выход вместе с PowerPoint-анимациями, видео и бесшовными переходами, но без повторного воспроизведения звука. Наложенный таймер повторяется в том же положении и масштабе не только поверх PPTX, но и поверх напрямую отрисованных PDF, видео и изображений.
...
- **4K, масштаб Windows и смена разрешения** — эфирное окно, PowerPoint, оба таймера и вспомогательные дисплеи используют актуальные физические границы каждого монитора. PDM автоматически перестраивает вывод после изменения разрешения, DPI, масштаба или расположения экранов, повторно закрепляет таймер внутри видимой области и скрывает панель задач именно на нужном дисплее.
