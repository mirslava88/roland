...
В панели задаются название события, начало и конец, текст над отсчётом, стоимость минуты, однотонный цвет, двухцветный градиент с настраиваемым углом либо фоновое изображение. Для крупного центрального значения выбирается ровно один из четырёх режимов: **«Текущее время»**, **«Таймер»**, **«До начала»** или **«До конца»**. У каждого режима собственный запоминаемый заголовок: переключение кнопок возвращает последний текст именно этого режима. Автоматические варианты используют формулировки «До начала мероприятия» и «До конца мероприятия», а кнопка **«Авто»** сбрасывает только заголовок текущего режима. Также можно отдельно скрыть или показать верхние часы, расписание, заголовок, название события, нижний остаток времени и итоговую стоимость. Нижнее значение «Осталось времени» всегда рассчитывается как разница между временем окончания мероприятия и текущими часами. Кнопки Play, Pause и Stop управляют только режимом **«Таймер»**; значения **«До начала»** и **«До конца»** независимо рассчитываются по системным часам. Время таймера корректируется кнопками `±1`, `±5`, `±10` минут либо тремя отдельными двухзначными полями часов, минут и секунд. Каждое поле принимает только две цифры, минуты и секунды ограничены значением `59`. Интерфейс Timer+ использует встроенный Inter Light, а крупное центральное время — Inter Thin, поэтому установка шрифта в системе не требуется.
