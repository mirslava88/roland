...
- Живая копия удерживает последний готовый кадр во время перехода на PowerPoint и автоматически открывает первый статичный слайд — дополнительное нажатие кликера больше не требуется.
- При переназначении главного эфирного дисплея во время показа PDM сначала переносит и проверяет реальный вывод, а уже затем меняет роли экранов: прежний главный монитор можно сразу назначить суфлёром, не оставляя новый главный экран пустым.
- Выбранная подложка заполняет свободные внешние дисплеи и используется как безопасный промежуточный кадр; панели задач на занятых PDM экранах повторно скрываются после смены источника.
- Наложенный таймер при Stop теперь паркует то же нативное окно вместо пересоздания: положение, размер и DPI сохраняются при следующем запуске таймера.
- При показе браузера PDM сохраняет исходное состояние окна и возвращает его на прежний монитор после выхода из эфира. Автоматическое изменение системной схемы дисплеев отключено.
- Подготовленные каналы восстанавливаются только после аварийного завершения; подтверждённое обычное закрытие начинает следующую сессию с пустой рабочей областью.
