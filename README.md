...
- **PDF и офисные документы** — сразу после добавления PDF в канал PDM в фоне готовит полноразмерные страницы под разрешение и DPI выбранного эфирного дисплея; небольшой безопасный отступ защищает края на проекторах и мониторах с обрезанием. Прогрессивная загрузка миниатюр и комбинированный рендер через Windows.Data.Pdf, PDFium и резервный pdf.js автоматически обходят проблемные страницы. Также доступен предпросмотр Word и Excel через установленный Microsoft Office.
...
Карандаш в правом верхнем углу карточки задаёт подпись канала. Текст располагается справа и растёт влево, поэтому номер и состояние канала остаются видимыми. Enter или щелчок вне поля сохраняет подпись, Escape отменяет изменение. PPTX и PDF после добавления в канал сразу начинают фоновую подготовку; статус **«Кэширование…»** исчезает, когда эфирные кадры готовы.
